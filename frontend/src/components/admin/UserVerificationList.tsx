...
const UserVerificationList: FC = () => {
  ...
  const [displayResidents, setDisplayResidents] = useState<DisplayResidentType>(
    {
      pagination: {
        limit: 10,
        currentPage: 0,
  ...
  const getVerificationRequest = useCallback(
    async (currentPage?: number, limit?: number, append: boolean = false) => {
      try {
        setPending(true);
        ...
        if (!error && data) {
          ...
          setDisplayResidents((prev) => ({
            ...
            result: append ? [...prev.result, ...data.result] : data.result,
          }));
        } else {
    ...
    [
      search,
      displayResidents.pagination.currentPage,
  ...
  const markAsVerified = async (userId: string | undefined) => {
    try {
      ...
      if (!error && displayResidents) {
        ...
        setDisplayResidents((prev) => ({
          ...prev,
          result: prev.result.filter((eachItem) => eachItem.id !== userId),
        }));
      }
    } catch (err) {
      console.log(err);
          ...
          {displayResidents.result.map((user, index) => (
            ...
            >
              ...
              <div className="flex w-full px-4 text-gray-400 font-medium justify-between">
                ...
                <span>{user.mealType ? MealToContent[user.mealType] : ""}</span>
              </div>
              <div className="flex w-full px-4 text-gray-400 font-medium justify-between">
                {user.gender && <span>{toTitleCase(user.gender)}</span>}
                <span>{user.isVeg ? "Vegetarian" : "Non-Vegetarian"}</span>
              </div>
              {user.residentialData && (
                <div className="flex w-full px-4 text-gray-400 font-medium justify-between">
                  <span>
                    {BuildingList[user.residentialData.building]?.name}
                  </span>
                  <span>{FloorList[user.residentialData.floor]?.name}</span>
                </div>
              )}
            </div>
          ))}
          ...
          <div className="  w-full max-w-xs p-4 bg-white rounded-md animate-slide-in">
            <div className=" flex flex-col items-center gap-4">
              <span className=" font-semibold text-center">
                ...
                <span className=" font-bold ml-1">{selectedUser.name}</span> ?
              </span>
            </div>
